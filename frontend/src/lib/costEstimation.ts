...
export function buildModelPricingByName(
  capabilities: ModelCapabilityRow[],
  ...
  for (const cap of capabilities) {
    ...
    const inCost =
      finiteOrNull(cap.usd_per_1m_tokens_in) ?? finiteOrNull(cap.input_cost_per_1m);
    const outCost =
      finiteOrNull(cap.usd_per_1m_tokens_out) ?? finiteOrNull(cap.output_cost_per_1m);
    const cachedInCost =
      finiteOrNull(cap.usd_per_1m_tokens_in_cached) ??
      finiteOrNull(cap.cached_input_cost_per_1m) ??
      inCost;
...
export function estimateConversationCostUsd(
  ...
  for (const usage of usageRows) {
    ...
    const boundedCached = Math.max(
      0,
      Math.min(Math.trunc(usage.cached_prompt_tokens), boundedPrompt),
    );
