...
export function getChatKnowledgeDraft(): ChatKnowledgeDraft {
  return get(knowledgeDraftStore);
}
...
export function chatToolDraftHasOverrides(): boolean {
  // Deliberately excludes the knowledge draft: forced retrieval is a single-shot
...
export function seedChatToolDraftFromUserMessage(entry: UserMessageEntry | null): void {
  ...
  // message. The knowledge draft deliberately does NOT: forced retrieval is a
