<script lang="ts">
  ...
  import {
    ...
    getKnowledgeGraphBuilders,
    getKnowledgeSources,
    getKnowledgeStorages,
    getKnowledgeStorageLog,
    ingestKnowledgeStorage,
    queryKnowledgeStorage,
    updateKnowledgeStorage,
    type CreateKnowledgeStorageInput,
    type EntitySourceInfo,
    type GraphBuilderInfo,
    type IngestResult,
    type KnowledgeGraphContext,
    type KnowledgeLogEntry,
    type KnowledgeQueryHit,
    type KnowledgeStorageInfo,
  ...
  let storages = $state<KnowledgeStorageInfo[]>([]);
  ...
  let queryHits = $state<Record<string, KnowledgeQueryHit[]>>({});
  let queryGraphs = $state<Record<string, KnowledgeGraphContext | null>>({});
  let logOpen = $state<Record<string, boolean>>({});
  let logEntries = $state<Record<string, KnowledgeLogEntry[]>>({});
    ...
    try {
      const sourceParams: Record<string, unknown> =
        entitySource === "files"
          ...
          : {};
      const input: CreateKnowledgeStorageInput = {
        name: name.trim(),
        entitySource,
      ...
      await createKnowledgeStorage(input);
      name = "";
      rootsText = "";
      graphBuilder = "";
      watchNew = false;
  ...
  async function ingest(id: string): Promise<void> {
    ...
    error = null;
    try {
      ingestResults = { ...ingestResults, [id]: await ingestKnowledgeStorage(id) };
      await load();
    } catch (e) {
      error = e instanceof Error ? e.message : String(e);
    } finally {
  ...
  async function runQuery(id: string): Promise<void> {
    ...
    try {
      ...
      const result = await queryKnowledgeStorage(id, q, topK, queryUseGraph[id] ? "graph" : "simple");
  ...
  async function toggleWatch(storage: KnowledgeStorageInfo): Promise<void> {
    busyId = storage.id;
    error = null;
    try {
      await updateKnowledgeStorage(storage.id, { watch: !storage.watch });
      await load();
    } catch (e) {
      error = e instanceof Error ? e.message : String(e);
    } finally {
  ...
  async function toggleLog(id: string): Promise<void> {
    ...
    if (!next) return;
    try {
      logEntries = { ...logEntries, [id]: (await getKnowledgeStorageLog(id)).entries };
    } catch (e) {
      error = e instanceof Error ? e.message : String(e);
    }
  }
  ...
  function logSummary(entry: KnowledgeLogEntry): string {
...
<main class="flex min-w-0 flex-col gap-3.5" data-testid="knowledge-section">
  {#if error}
    <div class="rounded-lg border border-destructive/40 bg-destructive/10 p-2 text-xs text-destructive" role="alert" data-testid="knowledge-error">
      {error}
    </div>
  {/if}
  ...
  {:else if storages.length === 0}
    <p class="text-sm text-muted-foreground" data-testid="knowledge-empty">No storages yet. Create one below, then ingest it.</p>
  {:else}
    <div class="flex flex-col gap-2.5">
      {#each storages as storage (storage.id)}
        {@const indexing = indexingTasks.get(storage.id)}
        <section class="rounded-lg border border-border bg-card p-3" data-testid="knowledge-storage" data-storage-name={storage.name}>
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                <div class="text-sm font-bold text-foreground" data-testid="knowledge-storage-name">{storage.name}</div>
                {#if storage.watch}
                  <span
                    ...
                    data-testid="knowledge-watch-badge">watching</span>
                {/if}
              </div>
              <div class="text-xs text-muted-foreground" data-testid="knowledge-storage-meta">
            ...
            <div class="flex shrink-0 gap-2">
              <button
                ...
                data-testid="knowledge-log-toggle"
              ...
              <button
                type="button"
                class="{ghostBtn} border-slate-300"
                data-testid="knowledge-watch-toggle"
              ...
              <button type="button" class="{ghostBtn} border-slate-300" data-testid="knowledge-ingest" disabled={busyId === storage.id || !!indexing} onclick={() => ingest(storage.id)}>
                {indexing ? "Indexing…" : busyId === storage.id ? "Working…" : "Ingest"}
              </button>
              <button type="button" class="{ghostBtn} border-destructive/40 text-destructive" data-testid="knowledge-delete" disabled={busyId === storage.id || !!indexing} onclick={() => remove(storage.id)}>
                Delete
              </button>
            </div>
          </div>
          ...
          {#if logOpen[storage.id]}
            <div class="mt-2 rounded-md border border-border bg-muted/40 p-2 text-xs" data-testid="knowledge-log">
              ...
              {:else}
                <ul class="flex flex-col gap-1">
                  {#each logEntries[storage.id] as entry (entry.id)}
                    <li class="flex items-baseline gap-2" data-testid="knowledge-log-entry" data-log-event={entry.event} data-log-actor={entry.actor}>
          ...
          {#if indexing}
            <div
              ...
              data-testid="knowledge-indexing"
            >
              ...
              {#if indexing.progress}
                <span class="tabular-nums text-primary/80" data-testid="knowledge-indexing-progress">{indexing.progress}</span>
              {/if}
              <button
                type="button"
          ...
          {#if ingestResults[storage.id]}
            ...
            <div class="mt-2 text-xs text-muted-foreground" data-testid="knowledge-ingest-result">
          ...
          <div class="mt-2.5 flex items-center gap-2">
            ...
            <input
              ...
              max="50"
              title="top k"
              data-testid="knowledge-query-topk"
            ...
            {#if storage.graph}
              <label class="flex shrink-0 items-center gap-1 text-xs text-muted-foreground">
                <input type="checkbox" data-testid="knowledge-query-graph"
                  checked={queryUseGraph[storage.id] ?? false}
                  onchange={(e) => (queryUseGraph = { ...queryUseGraph, [storage.id]: e.currentTarget.checked })} />
                graph
              </label>
            {/if}
            <button type="button" class="{ghostBtn} border-slate-300 shrink-0" data-testid="knowledge-test" disabled={busyId === storage.id} onclick={() => runQuery(storage.id)}>
              Test
            </button>
          </div>
          ...
          {#if queryHits[storage.id]}
            {#if queryHits[storage.id].length === 0}
              <div class="mt-2 text-xs text-muted-foreground" data-testid="knowledge-no-hits">No matches.</div>
            {:else}
              <ol class="mt-2 flex flex-col gap-1.5">
                {#each queryHits[storage.id] as hit, i (i)}
                  <li class="rounded-md border border-border bg-muted/40 p-2 text-xs" data-testid="knowledge-hit" data-hit-origin={hit.origin}>
                    <div class="mb-0.5 flex justify-between gap-2 text-muted-foreground">
                      <code class="truncate" data-testid="knowledge-hit-source">{String(hit.metadata.relativePath ?? hit.sourceId)}</code>
                      <span class="flex shrink-0 items-center gap-1.5">
                        {#if hit.origin === "graph"}
                          <span class="rounded bg-primary/15 px-1 font-semibold text-primary" data-testid="knowledge-hit-origin">graph</span>
                        {/if}
                        {hit.score.toFixed(4)}
                      </span>
                    </div>
            ...
            {#if queryGraphs[storage.id]}
              ...
              <div class="mt-2 rounded-md border border-border bg-muted/40 p-2 text-xs" data-testid="knowledge-graph-context">
                ...
                <ul class="flex flex-col gap-0.5 text-muted-foreground">
                  {#each g.relations as rel, i (i)}
                    <li data-testid="knowledge-graph-relation">
  ...
  {#if !showCreate}
    <div>
      <button type="button" class={ghostBtn} data-testid="knowledge-add" onclick={() => (showCreate = true)}>
        + Add storage
      </button>
    </div>
  {:else}
  <section class="rounded-lg border border-border bg-card p-3">
    ...
    <div class="grid grid-cols-2 gap-2.5">
      <label class="flex flex-col gap-1 text-xs">
        ...
        <input class={inputClass} data-testid="knowledge-name" bind:value={name} placeholder="My docs" />
      </label>
      <label class="flex flex-col gap-1 text-xs">
        <span class="font-semibold text-foreground">Entity source</span>
        <select class={inputClass} data-testid="knowledge-source" bind:value={entitySource}>
      ...
      <label class="flex flex-col gap-1 text-xs">
        ...
        <input class={inputClass} data-testid="knowledge-provider" bind:value={providerId} placeholder="openai / lmstudio" list="knowledge-provider-options" />
      </label>
      <label class="flex flex-col gap-1 text-xs">
        <span class="font-semibold text-foreground">Embedding model</span>
        <input class={inputClass} data-testid="knowledge-model" bind:value={model} placeholder="text-embedding-3-small" list="knowledge-embed-model-options" />
      </label>
      <datalist id="knowledge-provider-options">
        {#each llmProviders as p (p.id)}<option value={p.id}></option>{/each}
      </datalist>
      <datalist id="knowledge-embed-model-options">
        {#each embeddingModelOptions as m (m)}<option value={m}></option>{/each}
      </datalist>
      {#if providerId.trim() && embeddingModelOptions.length === 0}
        <p class="col-span-2 -mt-1 text-[11px] text-amber-700 dark:text-amber-400" data-testid="knowledge-no-models-hint">
      ...
      {#if entitySource === "files"}
        <label class="col-span-2 flex flex-col gap-1 text-xs">
          ...
          <textarea class="{inputClass} min-h-[64px] resize-y font-mono" data-testid="knowledge-roots" bind:value={rootsText}
            placeholder={"/workspace/docs\n/workspace/backend/src"}></textarea>
        </label>
      {/if}
      <label class="flex flex-col gap-1 text-xs">
        <span class="font-semibold text-foreground">Knowledge graph</span>
        <select class={inputClass} data-testid="knowledge-graph-builder" bind:value={graphBuilder}>
      ...
      <label class="flex items-center gap-1.5 self-end pb-1.5 text-xs text-foreground">
        <input type="checkbox" data-testid="knowledge-watch" bind:checked={watchNew} />
      ...
      {#if graphBuilder !== ""}
        <div class="grid grid-cols-2 gap-2.5">
          <label class="flex flex-col gap-1 text-xs">
            ...
            <input class={inputClass} data-testid="knowledge-graph-provider" bind:value={graphProviderId} placeholder="openai / lmstudio" list="knowledge-provider-options" />
          </label>
          <label class="flex flex-col gap-1 text-xs">
            <span class="font-semibold text-foreground">Extraction model</span>
            <input class={inputClass} data-testid="knowledge-graph-model" bind:value={graphModel} placeholder="gpt-4o-mini" list="knowledge-graph-model-options" />
          </label>
          <datalist id="knowledge-graph-model-options">
    ...
    <div class="mt-2.5">
      <button type="button" class="{ghostBtn} border-slate-300" data-testid="knowledge-create" disabled={!canCreate || creating} onclick={create}>
