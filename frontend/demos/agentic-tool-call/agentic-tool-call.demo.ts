import { defaultAgentId, PROBE_MESSAGE } from "../../e2e/api/client";
...
const TITLE_MS = 200;
const PARAMS_MS = 300;
const TOOL_ROUND_MS = 1500;
const FINAL_PLANNER_MS = 700;
const TOOL_PARAMS = "{}";
...
test("agentic-tool-call", async ({ app, request }) => {
  ...
  await expect(tool).toBeVisible({ timeout: E2E_LLM_TIMEOUT_MS });
  await expect(tool).toContainText("get_current_time");
