...
import { KNOWLEDGE_PROBE_MESSAGE, apiBaseUrl, defaultAgentId, getConversationEntries } from "./harness/client";
...
/**
 * knowledge end-to-end coverage through the running app (Playwright, stub-LLM mode).
...
async function makeDocs(files: Record<string, string>): Promise<string> {
  const dir = await mkdtemp(path.join(os.tmpdir(), "e2e-knowledge-docs-"));
...
function storageCard(page: Page, name: string) {
  return page.locator(`[data-testid="knowledge-storage"][data-storage-name="${name}"]`);
}
...
async function createStorage(
  ...
  await expect(page.getByTestId("knowledge-add").or(page.getByTestId("knowledge-name"))).toBeVisible();
  if (await page.getByTestId("knowledge-add").isVisible()) await page.getByTestId("knowledge-add").click();
  await page.getByTestId("knowledge-name").fill(name);
  await page.getByTestId("knowledge-provider").fill("stub");
  await page.getByTestId("knowledge-model").fill("stub-embed");
  await page.getByTestId("knowledge-roots").fill(roots);
  if (opts.graph) {
    await page.getByTestId("knowledge-graph-builder").selectOption("llm");
    await page.getByTestId("knowledge-graph-provider").fill("stub");
    await page.getByTestId("knowledge-graph-model").fill("stub-graph");
  }
  if (opts.watch) await page.getByTestId("knowledge-watch").check();
...
test("knowledge: create a files storage, ingest, retrieve the closest doc, delete", async ({ app }) => {
  const docs = await makeDocs({ "db.md": DB_DOC, "cooking.md": COOK_DOC });
  const name = `e2e-life-${Date.now()}`;
  try {
    await app.page.goto("/settings/knowledge");
    await expect(app.page.getByTestId("knowledge-section")).toBeVisible();
    const card = await createStorage(app.page, name, docs);

    await card.getByTestId("knowledge-ingest").click();
    await expect(card.getByTestId("knowledge-ingest-result")).toContainText("+2 added", { timeout: 10_000 });
    await expect(card.getByTestId("knowledge-storage-meta")).toContainText("2 chunks");

    await card.getByTestId("knowledge-query").fill("how do database migrations work in prisma");
    await card.getByTestId("knowledge-test").click();
    const top = card.getByTestId("knowledge-hit").first().getByTestId("knowledge-hit-source");
    await expect(top).toHaveText("db.md", { timeout: 10_000 });

    // Activity log: creation and the manual ingest, newest first, with stats.
    await card.getByTestId("knowledge-log-toggle").click();
    await expect(card.getByTestId("knowledge-log")).toBeVisible();
    const entries = card.getByTestId("knowledge-log-entry");
    ...
    await card.getByTestId("knowledge-delete").click();
...
test("knowledge: which document ranks first depends on the query", async ({ app }) => {
  ...
  try {
    ...
    const card = await createStorage(app.page, name, docs);
    await card.getByTestId("knowledge-ingest").click();
    await expect(card.getByTestId("knowledge-storage-meta")).toContainText("3 chunks", { timeout: 10_000 });

    const top = card.getByTestId("knowledge-hit").first().getByTestId("knowledge-hit-source");

    await card.getByTestId("knowledge-query").fill("tomato basil pasta recipe boil water");
    await card.getByTestId("knowledge-test").click();
    await expect(top).toHaveText("cooking.md", { timeout: 10_000 });

    await card.getByTestId("knowledge-query").fill("tcp socket packets network latency");
    await card.getByTestId("knowledge-test").click();
...
test("knowledge: re-ingest skips unchanged files and re-embeds changed ones", async ({ app }) => {
  const docs = await makeDocs({ "db.md": DB_DOC, "cooking.md": COOK_DOC });
  ...
  try {
    ...
    await card.getByTestId("knowledge-ingest").click();
    await expect(card.getByTestId("knowledge-ingest-result")).toContainText("2 skipped", { timeout: 10_000 });
    ...
    await card.getByTestId("knowledge-ingest").click();
    await expect(card.getByTestId("knowledge-ingest-result")).toContainText("1 updated", { timeout: 10_000 });
...
test("knowledge: a storage and its counts persist across a page reload", async ({ app }) => {
  const docs = await makeDocs({ "db.md": DB_DOC, "cooking.md": COOK_DOC });
  ...
  try {
    ...
    const card = await createStorage(app.page, name, docs);
    await card.getByTestId("knowledge-ingest").click();
    await expect(card.getByTestId("knowledge-storage-meta")).toContainText("2 chunks", { timeout: 10_000 });

    await app.page.reload();
    await expect(app.page.getByTestId("knowledge-section")).toBeVisible();
    const reloaded = storageCard(app.page, name);
    await expect(reloaded).toBeVisible({ timeout: 10_000 });
    await expect(reloaded.getByTestId("knowledge-storage-meta")).toContainText("2 chunks");

    await reloaded.getByTestId("knowledge-delete").click();
  } finally {
    await rm(docs, { recursive: true, force: true });
  }
});

test("knowledge graph: ingest extracts a graph; graph strategy pulls connected docs + context", async ({ app }) => {
  ...
  try {
    ...
    await card.getByTestId("knowledge-ingest").click();
    await expect(card.getByTestId("knowledge-ingest-result")).toContainText("+2 added", { timeout: 10_000 });
    await expect(card.getByTestId("knowledge-ingest-result")).toContainText("graph: 3 nodes / 2 edges");
    await expect(card.getByTestId("knowledge-storage-meta")).toContainText("graph (llm): 3 nodes / 2 edges");

    // topK=1 keeps the vector seed set to the lexically-matching doc alone, so
    // any second hit can only come from the graph walk.
    await card.getByTestId("knowledge-query").fill("alpha service ingress traffic");
    await card.getByTestId("knowledge-query-topk").fill("1");
    await card.getByTestId("knowledge-test").click();
    await expect(card.getByTestId("knowledge-hit").first().getByTestId("knowledge-hit-source")).toHaveText("graph-a.md", {
      timeout: 10_000,
    });
    await expect(card.locator('[data-testid="knowledge-hit"][data-hit-origin="graph"]')).toHaveCount(0);
    ...
    await card.getByTestId("knowledge-query-graph").check();
    await card.getByTestId("knowledge-test").click();
    const graphHit = card.locator('[data-testid="knowledge-hit"][data-hit-origin="graph"]');
    await expect(graphHit).toHaveCount(1, { timeout: 10_000 });
    await expect(graphHit.getByTestId("knowledge-hit-source")).toHaveText("graph-b.md");
    await expect(card.getByTestId("knowledge-graph-context")).toBeVisible();
    const relations = card.getByTestId("knowledge-graph-relation");
    await expect(relations).toHaveCount(2);
    await expect(card.getByTestId("knowledge-graph-context")).toContainText("Alpha Service");
    await expect(card.getByTestId("knowledge-graph-context")).toContainText("publishes to");
    await expect(card.getByTestId("knowledge-graph-context")).toContainText("Gamma Store");
...
test("knowledge chat: the agent explores a base and adds sources via the knowledge tool", async ({ app, request }) => {
  // Two tool rounds + a background ingest — needs more than the default budget.
  test.setTimeout(30_000);
  const base = await mkdtemp(path.join(os.tmpdir(), "e2e-knowledge-chatsrc-"));
  ...
  try {
    ...
    const createRes = await request.post(`${baseUrl}/api/knowledge/storages`, {
      data: {
        name,
        entitySource: "files",
    ...
    try {
      ...
      await expect(app.chat.transcript.toolRow()).toContainText("knowledge");
      ...
      await expect
        .poll(
          async () => {
            const infos = (await (await request.get(`${baseUrl}/api/knowledge/storages`)).json()) as Array<{
              id: string;
      ...
      const log = (await (
        await request.get(`${baseUrl}/api/knowledge/storages/${storage.id}/log`)
      ...
      await request.delete(`${baseUrl}/api/knowledge/storages/${storage.id}`);
    }
  } finally {
...
test("knowledge chat: the agent orients with list_storages and reads a full source", async ({ app, request }) => {
  test.setTimeout(30_000);
  const docs = await makeDocs({ "db.md": DB_DOC, "cooking.md": COOK_DOC });
  const baseUrl = apiBaseUrl();
  ...
  try {
    ...
    expect(createRes.ok()).toBeTruthy();
    const storage = (await createRes.json()) as { id: string };
    expect((await request.post(`${baseUrl}/api/knowledge/storages/${storage.id}/ingest`, { data: {} })).ok()).toBeTruthy();
    ...
    const original = agent.default_llm_configuration ?? null;
    const tools = { ...((original?.tools as Record<string, unknown>) ?? {}) };
    tools.knowledge = { policy: "allow", rules: { storages: [storage.id], top_k: 8, strategy: "simple" } };
    try {
      expect(
        (
          await request.put(`${baseUrl}/api/agents/${agentId}`, {
...
test("tool approval: editing knowledge params before approving is audited in the transcript", async ({ app, request }) => {
  ...
  const original = agent.default_llm_configuration ?? null;
  const tools = { ...((original?.tools as Record<string, unknown>) ?? {}) };
  tools.knowledge = { policy: "ask", rules: { storages: [], top_k: 8, strategy: "simple" } };
  ...
  try {
    ...
    await app.chat.gotoNew(agentId);
    await app.chat.userInput.typeMessage(KNOWLEDGE_PROBE_MESSAGE);
    await app.chat.userInput.send();
...
test("knowledge chat: the agent creates a storage on the fly from the configured template", async ({ app, request }) => {
  test.setTimeout(30_000);
  const base = await mkdtemp(path.join(os.tmpdir(), "e2e-knowledge-create-"));
  const put = async (rel: string, content: string) => {
    await mkdir(path.dirname(path.join(base, rel)), { recursive: true });
    await writeFile(path.join(base, rel), content);
  };
  ...
  try {
    ...
    // and was appended to the agent's own knowledge rules for future turns.
    await expect
      .poll(
        async () => {
          const infos = (await (await request.get(`${baseUrl}/api/knowledge/storages`)).json()) as Array<{
    ...
    const log = (await (
      await request.get(`${baseUrl}/api/knowledge/storages/${createdId}/log`)
    ).json()) as { entries: Array<{ event: string; actor: string; detail: Record<string, unknown> }> };
    ...
    const after = (await (await request.get(`${baseUrl}/api/agents/${agentId}`)).json()) as {
      default_llm_configuration?: { tools?: { knowledge?: { rules?: { storages?: string[] } } } };
    };
    expect(after.default_llm_configuration?.tools?.knowledge?.rules?.storages).toContain(createdId);
  } finally {
    await request.put(`${baseUrl}/api/agents/${agentId}`, {
      data: { name: agent.name, default_llm_configuration: original },
    });
    if (createdId) await request.delete(`${baseUrl}/api/knowledge/storages/${createdId}`);
    await rm(base, { recursive: true, force: true });
  }
});

test("knowledge watch: a watched storage auto-indexes on source changes, no Ingest click", async ({ app }) => {
  ...
  try {
    ...
    await expect(card.getByTestId("knowledge-watch-badge")).toBeVisible();
    ...
    await expect(card.getByTestId("knowledge-storage-meta")).toContainText("1 chunks / 1 sources", {
      timeout: 15_000,
    });

    // A new file under the watched root triggers a debounced re-index.
    await writeFile(path.join(docs, "cooking.md"), COOK_DOC);
    await expect(card.getByTestId("knowledge-storage-meta")).toContainText("2 chunks / 2 sources", {
      timeout: 15_000,
    });

    // Unwatch: badge clears and the toggle flips.
    await card.getByTestId("knowledge-watch-toggle").click();
    await expect(card.getByTestId("knowledge-watch-badge")).toHaveCount(0, { timeout: 10_000 });
...
test("knowledge: the storage card shows live indexing state while a slow ingest runs", async ({ app }) => {
  ...
  try {
    ...
    await card.getByTestId("knowledge-ingest").click();
    await expect(card.getByTestId("knowledge-indexing")).toBeVisible({ timeout: 10_000 });
    await expect(card.getByTestId("knowledge-ingest")).toBeDisabled();

    // The task finishes: live block clears, result + counts land.
    await expect(card.getByTestId("knowledge-indexing")).toHaveCount(0, { timeout: 15_000 });
    await expect(card.getByTestId("knowledge-ingest-result")).toContainText("+1 added");
    await expect(card.getByTestId("knowledge-storage-meta")).toContainText("1 chunks");
...
test("knowledge graph: the knowledge tool uses the graph strategy in chat", async ({ app, request }) => {
  const base = apiBaseUrl();
  ...
  const createRes = await request.post(`${base}/api/knowledge/storages`, {
    data: {
  ...
  expect(createRes.ok()).toBeTruthy();
  const storage = (await createRes.json()) as { id: string };
  const ingest = (await (await request.post(`${base}/api/knowledge/storages/${storage.id}/ingest`)).json()) as {
  ...
  try {
    ...
    await request.delete(`${base}/api/knowledge/storages/${storage.id}`);
    await rm(docs, { recursive: true, force: true });
  }
});

test("knowledge: an agent retrieves from a storage via the knowledge tool in chat", async ({ app, request }) => {
  const base = apiBaseUrl();
  const docs = await makeDocs({ "db.md": DB_DOC, "cooking.md": COOK_DOC });
  ...
  expect(createRes.ok()).toBeTruthy();
  const storage = (await createRes.json()) as { id: string };
  expect((await request.post(`${base}/api/knowledge/storages/${storage.id}/ingest`)).ok()).toBeTruthy();

  // Point the default agent's knowledge tool at the storage (policy: allow).
  const agentId = await defaultAgentId(request);
  const agent = (await (await request.get(`${base}/api/agents/${agentId}`)).json()) as {
    name: string;
    default_llm_configuration: Record<string, unknown> | null;
  ...
  try {
    ...
    // Ground-truth: the knowledge tool is actually enabled on the agent post-PUT.
    const verify = (await (await request.get(`${base}/api/agents/${agentId}`)).json()) as {
      default_llm_configuration?: { tools?: { knowledge?: { policy?: string } } };
    };
    expect(verify.default_llm_configuration?.tools?.knowledge?.policy).toBe("allow");

    await app.chat.gotoNew(agentId);
    await app.chat.userInput.typeMessage(KNOWLEDGE_PROBE_MESSAGE);
    await app.chat.userInput.send();

    // The planner calls the knowledge tool, it runs, and the conversation finalizes.
    await app.chat.transcript.waitForToolState("done");
    await expect(app.chat.transcript.toolRow()).toContainText("knowledge");
    await expect(app.chat.transcript.assistantMessage).toContainText("Prisma migration", { timeout: 15_000 });
  } finally {
