...
function parseOptionalModelReference(
  raw: unknown
): { provider_id?: string; model_name?: string } | null {
  if (raw == null) return null;
  const parsed = AgentModelReferenceSchema.safeParse(raw);
  if (!parsed.success) return null;
  return {
    provider_id:
      typeof parsed.data.provider_id === "string"
        ? parsed.data.provider_id
        : undefined,
    model_name:
      typeof parsed.data.model_name === "string"
...
export function applyAgentInputPatch(
  ...
  return {
    ...
    default_llm_configuration:
      patch.default_llm_configuration ?? fallback.default_llm_configuration,
    default_model_preset_id:
      patch.default_model_preset_id ?? fallback.default_model_preset_id,
    model_reference:
      parseOptionalModelReference(patch.model_reference) ??
      fallback.model_reference,
  };
}
