...
import type { ChunkRef, StoredGraphEdge } from '../store/knowledge-store.types.js';
import type { KnowledgeGraphContext, KnowledgeQueryHit, KnowledgeRetrieveResult } from '../contracts/knowledge.js';
...
type ModelBucket = {
  providerId: string;
  model: string;
  storages: Array<{ id: string; name: string; store: KnowledgeStore }>;
};
...
export class RetrieverService {
  ...
  async retrieve(input: RetrieveInput): Promise<KnowledgeQueryHit[]> {
  ...
  async retrieveGraph(input: GraphRetrieveInput): Promise<KnowledgeRetrieveResult> {
    const topK = Math.max(0, Math.floor(input.topK));
    ...
    const seeds: KnowledgeQueryHit[] = [];
    const graphOnly: KnowledgeQueryHit[] = [];
    const entities: KnowledgeGraphContext['entities'] = [];
    const relations: KnowledgeGraphContext['relations'] = [];
    ...
    const graph: KnowledgeGraphContext | null =
      entities.length > 0
        ? {
...
function hitRef(hit: KnowledgeQueryHit): ChunkRef {
