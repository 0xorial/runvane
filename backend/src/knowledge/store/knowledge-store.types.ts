/** Persisted descriptor for one knowledge storage (one .sqlite file). */
...
export type KnowledgeLogActor = 'user' | 'agent' | 'watcher';

export type KnowledgeLogEvent = 'created' | 'ingest' | 'ingest_failed' | 'source_added' | 'watch_changed';

export type KnowledgeLogEntry = {
  id: number;
  at: string;
  event: KnowledgeLogEvent;
  actor: KnowledgeLogActor;
