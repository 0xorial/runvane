...
import type { KnowledgeLogActor, StorageManifest } from './knowledge-store.types.js';
import type { KnowledgeStorageInfo } from '../contracts/knowledge.js';

/** DI token for the knowledge data directory (one .sqlite file per storage). */
export const KNOWLEDGE_DATA_DIR = Symbol('KNOWLEDGE_DATA_DIR');
...
/**
 * Owns the directory of knowledge storages. Each storage is a separate SQLite file
 ...
 * — this is the "separate part that builds the knowledge database".
 */
@Injectable()
export class StorageRegistry implements OnModuleDestroy {
  private readonly dir: string;
  private readonly cache = new Map<string, KnowledgeStore>();

  constructor(@Optional() @Inject(KNOWLEDGE_DATA_DIR) dir?: string) {
    this.dir = dir ?? process.env.KNOWLEDGE_DATA_DIR ?? path.resolve(process.cwd(), '.knowledge');
  ...
  open(id: string): KnowledgeStore | null {
    ...
    const store = new KnowledgeStore(this.fileFor(id));
  ...
  info(id: string): KnowledgeStorageInfo | null {
  ...
  listInfos(): KnowledgeStorageInfo[] {
    return this.listIds()
      .map((id) => this.info(id))
      .filter((x): x is KnowledgeStorageInfo => x !== null);
  }
