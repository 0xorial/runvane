...
| RV-015 | Conversation sidebar must support collapsible folder groups with conversation counts and a “Move to group” menu action (existing/new group). | `frontend/src/components/ConversationSidebar.tsx`, `backend/src/routes/conversations.ts`, `backend/src/infra/repositories/conversationsRepo.ts` |
| RV-016 | Conversation groups must be normalized in a dedicated `conversation_groups` table and conversations must reference groups by `group_id`. | `backend/migrations/0006_conversation_groups_table.sql`, `backend/src/infra/repositories/conversationsRepo.ts` |
