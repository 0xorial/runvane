...
import { MAX_UPLOAD_BYTES, type UploadFileResponse } from './uploads.types.js';
...
export class UploadsController {
  ...
  async upload(@UploadedFile() file: Express.Multer.File | undefined): Promise<UploadFileResponse> {
    ...
    if (file.size > MAX_UPLOAD_BYTES) throw new BadRequestException('file exceeds 10MB limit');
