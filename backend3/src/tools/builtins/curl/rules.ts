...
const CurlToolRulesSchema = z
  .object({
    ...
    blockedHosts: HostListSchema.default(DEFAULT_BLOCKED),
...
export function curlRulesSchema(): Record<string, unknown> {
  return {
    ...
    properties: {
      ...
      allowHttp: { type: 'boolean', default: false, description: 'Allow plain HTTP (otherwise HTTPS only).' },
      allowedHosts: {
        type: 'array',
        items: { type: 'string' },
        default: [],
      ...
      maxTimeoutMs: { type: 'integer', minimum: 100, maximum: 60000, default: 10000 },
